...
func MakeMatches(p *mmfHarness.MatchFunctionParams) ([]*pb.Match, error) {
	tickets, ok := p.PoolNameToTickets["Everyone"]
	if !ok {
		return nil, errors.New("Expected pool named Everyone.")
	}
	...
	for i := 0; i+1 < len(tickets); i += 2 {
		matches = append(matches, &pb.Match{
			MatchId:       fmt.Sprintf("profile-%s-time-%s-num-%d", p.ProfileName, t, i/2),
